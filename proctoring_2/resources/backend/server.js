...
// Keywords allowed for ALL subjects (Core Educational Tools)
const GLOBAL_ALLOWED = [
  'colab', 'google colab', 'classroom', 'new tab', 'google search', 'rknec.edu', 'smartlab'
];

const RULES = {
  ML: ['explorer', 'file browser', 'code', 'visual studio',
       'python', 'jupyter', 'anaconda', 'terminal', 'cmd', 'powershell',
       'notepad', 'sublime', 'pycharm', 'spyder', 'idle'],
  DBMS: ['sql plus', 'sqlplus', 'sqldeveloper', 'oracle', 'gmail',
...
io.on('connection', (socket) => {
  ...
  socket.on('ACTIVITY_UPDATE', (payload) => {
    ...
    for (let i = 0; i < (windows || []).length; i++) {
      ...
      // Check if this window/process is allowed
      let isAllowed = false;

      // 1. Check Global Allowed (Always-OK tools)
      for (const keyword of GLOBAL_ALLOWED) {
